...
_(若只需静默同步数据库，可直接在终端执行 `python -m reborn_core sync`)_
...
├── src/
│   └── reborn_core/           # 🧠 核心命名空间
│       ├── interfaces/        # 🔌 接入层 (API/Workers)
│       ├── domains/           # 🧠 核心业务域 (Brain/Memory/Voice)
│       ├── infrastructure/    # 🛠️ 基础设施层 (Database/Obsidian/Knowledge)
│       ├── core/              # ⚙️ 全局基建 (Config/Logger/Exceptions)
│       ├── utils/             # 🛠️ 通用工具类
│       └── scripts/           # 🛠️ 自动化运维脚本
├── data/                      # 🗄️ 物理持久化层
└── logs/                      # 📝 系统运行日志
